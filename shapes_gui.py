...
# citation: https://devhubcommunity.hashnode.dev/displaying-gifs-in-tkinter-python
    ...
    def calculate():
        try:
            ...
            row = ["Sphere", color, r, x, y, z, round(s.volume(), 2), round(s.surface_area(), 2)]
            data = ",".join(map(str, row))

            encrypted = encrypt_data(data)

            with open("shapes_data_encrypted.csv", "ab") as file:
            ...
            s.draw()
# execption handelling - bonus mark
        except ValueError:
...
Label(master, text="This is the main window").grid(row=0, column=12, columnspan=2, pady=10)
Button(master, text="Create Sphere", width=15, command=open_sphere_window).grid(row=1, column=6, columnspan=2, pady=10)
Button(master, text="Create Horn", width=15, command=open_horn_window).grid(row=2, column=12, columnspan=2, pady=10)
